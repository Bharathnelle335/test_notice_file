...
def is_git_host(url: str) -> bool:
    ...
    return (
        ("github.com" in u)
        or ("gitlab.com" in u)
        or ("bitbucket.org" in u)
        or u.startswith(("git://", "git@", "ssh://"))
    )
...
def extract_from_purl(purl: str | None):
...
def pick_spdx_license_from_spdx(pkg: dict) -> str | None:
    return (pkg.get("licenseConcluded") or pkg.get("licenseDeclared") or None)

def pick_spdx_license_from_cdx(comp: dict) -> str | None:
...
def _git_url_from_pkgish_locator(locator: str | None) -> str | None:
    if not locator:
        return None
    s = locator.strip()
...
def best_git_url_from_spdx(pkg: dict) -> str | None:
    if not pkg:
        return None
...
def best_git_url_from_cdx(comp: dict, purl: str | None) -> str | None:
    if comp:
...
def yaml_esc(s) -> str:
    if s is None:
        return '""'
    s = str(s)
    # Quote if YAML-sensitive chars or leading/trailing whitespace
...
def cmd_split(_args):
    ...
    for i, part in enumerate(parts):
        ...
        idxs.append(str(i))
    # communicate outputs to GITHUB_OUTPUT if present
...
def cmd_scan_part(args):
    ...
    for item in comps:
        ...
        if not name or not git_url:
            ...
            continue
        dest = os.path.join(
            f"repos_part_{part_index}",
            sanitize(name) + (f"-{sanitize(version)}" if version else ""),
        )
...
def cmd_aggregate_part(args):
    idx = str(args.part_index)
    with open(f"artifacts/parts/components_part_{idx}.json", "r", encoding="utf-8") as f:
        comps = json.load(f).get("components", [])
    def load_raw(name):
        p = os.path.join(f"scancode_results_part_{idx}", sanitize(name) + ".json")
        if os.path.exists(p):
    ...
    ensure_dir("artifacts")
    with open(f"artifacts/component_copyrights_part_{idx}.json", "w", encoding="utf-8") as f:
        json.dump({"components": results}, f, indent=2, ensure_ascii=False)
    print(f"Wrote artifacts/component_copyrights_part_{idx}.json with {len(results)} components.")

def cmd_build_part_attr(args):
    idx = str(args.part_index)
    with open(f"artifacts/parts/components_part_{idx}.json", "r", encoding="utf-8") as f:
        components = json.load(f).get("components", [])
    with open(f"artifacts/component_copyrights_part_{idx}.json", "r", encoding="utf-8") as f:
    ...
    for comp in components:
        ...
        lines.append(" - component: " + yaml_esc(name))
        if version is not None:
            lines.append("   version: " + yaml_esc(version))
        lines.append("   url: " + yaml_esc(url))
        lines.append("   license: " + yaml_esc(lic))
        cps = (by_name.get(name, {}) or {}).get("copyrights", {}).get("unique_statements", []) or []
        if cps:
            lines.append("   copyrights:")
            for c in cps:
                lines.append("     - " + yaml_esc(c))
        else:
            lines.append("   copyrights: []")

    ensure_dir("artifacts")
    with open(f"artifacts/component_attribution_part_{idx}.yaml", "w", encoding="utf-8") as f:
        f.write("\n".join(lines) + "\n")
    ...
    for comp in components:
        ...
        url = comp.get("git_url") or ""
        lic = comp.get("license") or ""
        t.append(f"Component: {name}")
        if version:
            t.append(f"Version: {version}")
        t.append(f"URL: {url}")
        t.append(f"License: {lic}")
        t.append("Copyrights:")
        cps = (by_name.get(name, {}) or {}).get("copyrights", {}).get("unique_statements", []) or []
        if cps:
            for s in cps:
            ...
            t.append("- (none)")
        t.append("")
    with open(f"artifacts/component_attribution_part_{idx}.txt", "w", encoding="utf-8") as f:
        f.write("\n".join(t).rstrip() + "\n")
    print(f"Wrote per-part attribution YAML/TXT for part {idx}.")
...
def cmd_build_progress(_args):
    ...
    for c in progress:
        ...
        if version is not None:
            yl.append("   version: " + yaml_esc(version))
    ...
    with open("artifacts/component_attribution_progress.txt", "w", encoding="utf-8") as f:
        f.write("\n".join(tl).rstrip() + "\n")
    print(f"Progress: {len(progress)} components merged so far.")
...
def cmd_merge_final(_args):
    ...
    print(f"Merged final copyrights for {len(out)} components.")
...
def main():
    ...
    sp.add_parser("split")
    p_scan = sp.add_parser("scan-part")
    p_scan.add_argument("--part-index", required=True)
    p_scan.add_argument("--scancode", required=True)

    p_agg = sp.add_parser("aggregate-part")
    p_agg.add_argument("--part-index", required=True)

    p_attr = sp.add_parser("build-part-attr")
    p_attr.add_argument("--part-index", required=True)
    ...
    try:
        if args.cmd == "generate":
            ...
            cmd_split(args)
        elif args.cmd == "scan-part":
            cmd_scan_part(args)
        elif args.cmd == "aggregate-part":
            cmd_aggregate_part(args)
        elif args.cmd == "build-part-attr":
            cmd_build_part_attr(args)
        elif args.cmd == "build-progress":
            cmd_build_progress(args)
        elif args.cmd == "merge-final":
            cmd_merge_final(args)
        elif args.cmd == "build-final-attr":
            cmd_build_final_attr(args)
        else:
